...
public class StatementService(McbaContext context) : IStatementService
{
    ...
    public async Task<(int totalPageCount, IEnumerable<Transaction> transactions)> GetPaginatedAccountTransactions(int accountNumber, (int pageNumber, int itemCount)? pagination = null)
    {
        var allTransactionsQuery = _dbContext.Transactions
        ...
        if (pagination.HasValue)
        {
            ...
            var totalPageCount = (int)Math.Ceiling((await allTransactionsQuery.CountAsync()) / (decimal)pagination.Value.itemCount);
            return (
                totalPageCount,
                await allTransactionsQuery.Skip((pagination.Value.pageNumber - 1) * pagination.Value.itemCount)
